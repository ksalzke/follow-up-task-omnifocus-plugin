...
(() => {
  const action = new PlugIn.Action(async function (selection, sender) {
    ...
    const dependencies: Task[] = dependencyLibrary ? dependencyLibrary.getDependents(selectedTask) : []
    const dependencyString = dependencies.length > 0 ? `\n\n DEPENDENT: \n - ${dependencies.map(task => task.name).join('\n - ')}` : '\n\nDEPENDENT: None'

    const prerequisites: Task[] = dependencyLibrary ? dependencyLibrary.getPrereqs(selectedTask) : []
    ...
    if (form.values.propertiesToTransfer === 'all_inc_dependencies') {
      for (const prereq of prerequisites) {
        ...
        await dependencyLibrary.removeDependency(prereq.id.primaryKey, selectedTask.id.primaryKey)
      }

      for (const dep of dependencies) {
        await dependencyLibrary.addDependency(newTask, dep)
        await dependencyLibrary.removeDependency(selectedTask.id.primaryKey, dep.id.primaryKey)
      }
    }
